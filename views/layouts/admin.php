...
<?php $this->beginBody() ?> 
            ...
            <div class="col-md-6">    
                <?php if(!Yii::$app->user->isGuest) : ?>
                   <div class="menu-link  hidden-xs">
                        ...
                        <?php $unread = YBoardMessage::find()->inboxScope()->unreadScope()->andWhere(['sendto' =>\Yii::$app->user->id])->count(); ?>
                        <?php $total = YBoardMessage::find()->inboxScope()->andWhere(['sendto' =>\Yii::$app->user->id])->count(); ?>
    ...
    <div class="container">
        <?php if(isset($this->params['adminMenu'])): ?>
            <div class="col-md-2"> 
                <div class="row">
                     <ul class="list-group col-md-12">
                        <li class="header4 header2-style"><?= isset($this->params['adminMenu']['title'])?$this->params['adminMenu']['title']:$this->title ?></li>
                        <?php foreach($this->params['adminMenu'] as $item): ?>
                            <?php if(isset($item['label']) && isset($item['url'])): ?>
                                <li class="list-group-item"><?= Html::a($item['label'], $item['url']) ?></li>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </ul>
                </div>
